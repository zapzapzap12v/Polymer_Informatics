...
def run_ansys_batch_simulation(dataset_path):
    ...
    for idx, row in df.head(10).iterrows():
        try:
            ...
            thickness = row['Thickness_um'] * 1e-6  # convert to meters
            k_val = row['Dielectric_Constant']
            ...
            c_theoretical = (8.854e-12 * k_val * (0.01 * 0.01)) / thickness
            simulated_capacitance.append(c_theoretical * (1 + 0.01 * np.random.randn()))
            
            leakage = (100 / thickness) * row['Dissipation_Factor'] * 1e-9
